...
pub(super) fn ordering(
    ...
    let mut best_order = None::<Order>;
    let mut best_crossings = usize::MAX;

    for order in init_orders(graph) {
        let reduced = reduce_crossings_bilayer_sweep(graph, order, cm_method, transpose);
        let crossings = reduced.crossings(graph);
        if crossings < best_crossings {
            best_crossings = crossings;
            best_order = Some(reduced);
        }
    }

    best_order
        .expect("crossing reduction produced no ordering")
        ._inner
}
...
fn init_orders(graph: &StableDiGraph<Vertex, Edge>) -> Vec<Order> {
    info!(
        target: "crossing_reduction",
        "Initializing order of vertices in each rank via source/sink walks."
    );
    let max_rank = graph
        .node_weights()
    ...
    [
        build_initial_order(graph, max_rank, true),
        build_initial_order(graph, max_rank, false),
    ]
    .into_iter()
    .collect()
}
...
fn build_initial_order(
    graph: &StableDiGraph<Vertex, Edge>,
    max_rank: usize,
    walk_from_sources: bool,
) -> Order {
    let mut order = vec![Vec::new(); max_rank + 1];
    let mut visited = HashSet::new();
    let mut queue = VecDeque::new();

    let mut seed_queue = |predicate: fn(&StableDiGraph<Vertex, Edge>, NodeIndex) -> bool| {
        for v in graph.node_indices() {
            if !visited.contains(&v) && predicate(graph, v) {
                visited.insert(v);
                queue.push_back(v);
                while let Some(next) = queue.pop_front() {
                    order[graph[next].rank as usize].push(next);
                    let neighbors = if walk_from_sources {
                        graph.neighbors_directed(next, Outgoing).collect::<Vec<_>>()
                    } else {
                        graph.neighbors_directed(next, Incoming).collect::<Vec<_>>()
                    };
    ...
    seed_queue(if walk_from_sources {
        |graph, node| graph.neighbors_directed(node, Incoming).next().is_none()
    } else {
        |graph, node| graph.neighbors_directed(node, Outgoing).next().is_none()
    });
...
fn transpose(graph: &StableDiGraph<Vertex, Edge>, order: &mut Order, move_down: bool) {
    ...
    while improved {
        ...
        for r in iterate(iter_dir, order.max_rank()) {
            ...
            let rank_len = order._inner[r].len();
            if rank_len < 2 {
                continue;
            }
            for i in 0..rank_len - 1 {
