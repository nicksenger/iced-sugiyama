...
impl Vertex {
    pub(crate) fn new(id: usize, size: (f64, f64)) -> Self {
        Self {
            id,
            size,
            ..Default::default()
...
pub(crate) fn start(mut graph: StableDiGraph<Vertex, Edge>, config: &Config) -> Layouts<usize> {
...
fn build_layout(mut graph: StableDiGraph<Vertex, Edge>, config: &Config) -> Layout {
    ...
    let layers = execute_phase_2(
        ...
        config
            .dummy_vertices
            .then_some(config.dummy_size * config.vertex_spacing),
