    ...
    ClusterLabel: Fn(usize, &Cluster) -> String,
{
    ...
    let mut sorted_clusters = layout.clusters.clone();
    ...
    for (position, node) in nodes.iter().copied().enumerate() {
        let Some(&(x, y)) = layout.coords.get(&position) else {
            continue;
        };
    ...
    let mut sorted_edges = layout.edges.clone();
    ...
    for edge in sorted_edges {
        ...
        if let (Some(label), Some((label_x, label_y))) =
            (edge.label.as_deref(), edge.label_position)
        {
            let _ = write!(
                plain,
                " {} {} {}",
